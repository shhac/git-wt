...
pub fn execute(allocator: std.mem.Allocator, command_args: []const []const u8, _: bool, _: bool, current_mode: mode_mod.Mode) !void {
    ...
    // Build flags array
    try stdout.writeAll("    local -a flags=()\n");
    if (no_tty) try stdout.writeAll("    flags+=(--no-tty)\n");
    if (non_interactive) try stdout.writeAll("    flags+=(--non-interactive)\n");
    if (plain) try stdout.writeAll("    flags+=(--plain)\n");
    ...
    if (parent_dir) |dir| {
        if (std.mem.indexOf(u8, dir, "{repo}") != null) {
            ...
            try stdout.writeAll("        flags+=(--parent-dir \"$parent_dir\")\n");
            try stdout.writeAll("    fi\n");
        } else {
            // Static parent dir
            const escaped_dir = try escapeShellString(allocator, dir);
            defer allocator.free(escaped_dir);
            try stdout.print("    flags+=(--parent-dir \"{s}\")\n", .{escaped_dir});
        }
    }
