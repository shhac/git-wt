...
| `rm [branch...]` | Remove worktree(s); interactive multi-select if no args. Every target is checked for uncommitted work before anything is deleted, so a dirty worktree can't leave you half-removed; interactively you pick which to remove anyway (default: skip). Deletion is parallel with a live progress line, and copes with read-only/immutable files that make `git worktree remove` die half-way. Also accepts the name of a leftover directory under the trees dir (the debris of an interrupted removal). Flags: `--keep-branch`, `--delete-branch`, `--force`, `--force-branch`. |
