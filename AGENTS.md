...
internal/
  cli/                             # cobra subcommands + per-command helpers
    ...
    list.go new.go add.go eject.go # the commands
    go_cmd.go rm.go rm_dirty.go rm_locked.go rm_remove.go clean.go
    lock.go alias.go config_cmd.go completion.go
    emit.go                        # fd<N> / bare-mode path emission
    picker.go                      # adapts internal/picker to worktrees
    worktrees.go                   # findByBranch, findNamedWorktree, filterOutCurrent, ...
    config_resolve.go              # layer git-config wt.* under CLI flags
    completion_helpers.go          # ValidArgsFunction candidates for go/rm/add/lock/unlock
  picker/                          # bubbletea SelectOne / SelectMany / Confirm
  wt/                              # worktree types, parsing, validation, lock times
...
| `rm [branch...]` | Remove worktree(s). Interactive multi-select if no args. Dirty preflight (`rm_dirty.go`) scans every target before anything is deleted: non-interactive refuses naming them all, interactive offers a multi-select over the dirty ones (toggle nothing = skip all). Confirm step is also the type-of-rm choice (worktree only / worktree+branch / cancel). Bounces to main if you remove your current worktree — emitted from a defer so a mid-list failure still moves the shell. Fast path (`rm_remove.go`): per-target clean-check → rename aside → `worktree prune` → parallel permission-fixing delete (`wt.DeleteTree`) with a TTY progress line; falls back to `git worktree remove` for edge cases (and for a locked target, should one get past the preflight). Arg resolution (`findNamedWorktree`, then rm-only rescue): `findByBranch` (exact, then unique-suffix) -> `findByTreesDirLeaf` (registered worktree at `<treesDir>/<arg>`, the only handle a detached worktree has) -> `orphanRmTarget` (unregistered leftover dir, rescue). Duplicate args are deduped by path. Lock preflight (`rm_locked.go`) runs before the dirty one, same shape: non-interactive refuses naming them all (regardless of `--force` — a lock is released, not overridden), interactive offers a multi-select of which to unlock and remove. Chosen targets are unlocked in `removeTargets` just before their own removal, so a cancel at the confirm step leaves every lock in place. |
| `go [branch]` | Navigate. Direct branch lookup with unique-suffix fallback (`auth` → `paul/auth`). Interactive picker if no arg. |
| `list` (`ls`) | Print the table. Columns: marker, branch, location (`#name` inside `.worktrees/`, rel-to-repo elsewhere, abs outside), mtime; locked rows add lock age + truncated reason (`lockTag`, shared with the pickers). Lock age is the mtime of `<commonDir>/worktrees/<id>/locked`, mapped back via each admin dir's `gitdir` file (`wt.attachLockTimes`). |
| `lock [branch...]` / `unlock [branch...]` | Thin wrappers over `git worktree lock/unlock` with gwt name resolution (`findNamedWorktree`: branch, suffix, trees-dir leaf) and a multi-select over the worktrees whose state would change. Already in the target state is a note, not an error. No PID/staleness detection — the reason is shown verbatim and the user judges. |
| `clean` | Remove worktrees whose local branch is gone OR whose upstream is gone. Both run by default; narrow with `--orphaned-only` / `--upstream-gone-only`. Dirty and locked worktrees are reported and skipped, never prompted for — it's a bulk sweep over worktrees the user never named. |
...
| `completion <shell>` | Print a Cobra-generated completion script for bash/zsh/fish/powershell. Dynamic argument completion via `ValidArgsFunction` on `go` / `rm` / `add` / `lock` / `unlock`. |
